...
class nicoDL_mainWindow(wx.Panel):
    ...
    def download(self):
        ...
        various.reflesh_libraryfile(os.path.join(nicodl.cdir, "data", "library_ALL.ndl"))
        print u"ライブラリリフレッシュ終了!"
        # RSS
        print u"RSSチェック中..."
        various.rsscheck(os.path.join(nicodl.cdir, "data"))
        print u"RSSチェック完了!"

        # DL部分
        f = open(os.path.join(nicodl.cdir, "data", "library_ALL.ndl"))
        lib = pickle.load(f)
        f.close()
        #print lib
        for item in lib:
            ...
            if item == None:
                ...
                # display
                nicodl.movie_id_display = item["movie_id"]  #
                wx.PostEvent(self, evt)
                ...
                try:
                    movie_path = self.movie_dl(nico, nicodl.nicoid, nicodl.nicopw, nicodl.savedir, mylist=mylistflag)
                except DownloadInterraptionException:
                ...
                various.rewrite_library(key="thumbnail", value=thumbnail, libraryfilepath=os.path.join(nicodl.cdir, "data\\library_ALL.ndl"),movie_id=item["movie_id"])
                various.rewrite_library(key="state", value=True, libraryfilepath=os.path.join(nicodl.cdir, "data\\library_ALL.ndl"),movie_id=item["movie_id"])
                various.rewrite_library(key="movie_path", value=movie_path, libraryfilepath=os.path.join(nicodl.cdir, "data\\library_ALL.ndl"),movie_id=item["movie_id"])
                ...
                if item["mylist_id"] != None:
                    ...
                    for i in my:
                        if i["mylist_id"] == item["mylist_id"]:
                            various.rewrite_library(key="downloaded", value=item["movie_id"], libraryfilepath=os.path.join(nicodl.cdir, "data\\library_ALL.ndl"),mylist_id=item["mylist_id"])

                # メール送信
                if self.mailcheck:
                    print "".join(["[",item["movie_id"], "]",u" 完了メール送信"])
                    mail = SendMail.nicodl_sendmail(nicodl.gid, nicodl.gpw, nicodl.toaddr)
                    mail.main("".join([title, "\n\n", description]), item["movie_id"])
                #nicodl.extention()  # 拡張
                # 外部表示用アトリビュートリセット
