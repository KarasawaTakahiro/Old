...
EVT_TYPE_DL_END = wx.NewEventType()
EVT_DL_END = wx.PyEventBinder(EVT_TYPE_DL_END)
...
class RemoteNicovideoDL(threading.Thread):
    ...
    def geturl(self):
        ...
        while True:
            try:
                ...
                logging.error(mess)
                if c > 10:
                    self.error.put((001, "Gmailに接続できませんでした."))
                    return
                elif c <= 10:
                    self.error.put((001, "Gmailに接続できませんでした.\nリトライします."))
                    time.sleep(600)
        ...
        try:
            ...
            self.error.put((002, "Gmailにログインできませんでした."))
            return
    ...
    def checkdir(self):
        ...
        if os.path.exists(self.savedir):
            ...
            self.savedir = '\\movie'
            if not os.path.exists(self.savedir):  # なければ作成
    ...
    def run(self):
        ...
        if myformat == False:  # Gamilが空
            ...
            try:
                ...
                self.error.put((010, u"ライブラリファイルが見つかりません."))
                return
        ...
        for item in lib:
            ...
            if item == None:
                ...
                try:
                    ...
                    self.error.put((020, "ニコニコ動画ににアクセスできませんでした."))
                    return
                ...
                try:
                    ...
                    self.error.put((030, u"ニコニコ動画にログインできませんでした."))
                    return
                ...
                try:
                    ...
                    mail.main(u"".join([title, "\n\n", description]), item["movie_id"])
                except:
                    self.error.put((002, u"Gmailにログインできませんでした."))
                # 外部表示用アトリビュートリセット
                self.movie_id_display = u"動画ID"
                self.title_display = u"タイトル"
                self.description_display = u"動画説明"
...
class nicoDL(wx.Frame):
    def __init__(self, size=wx.DefaultSize, pos=wx.DefaultPosition, *args, **kwds):
        ...
        self.Bind(EVT_DL_END, self.remotenicovideodl)
        ...
        # main
        print u"nicoDL 開始"
        if self.loadtest() == True:
            self.remotenicovideodl()
            ###self.main = RemoteNicovideoDL(self, os.getcwd(), debug=False)
    #
    def remotenicovideodl(self, evt=None):
            self.main = RemoteNicovideoDL(self, self.cdir, debug=False)
            try:
                self.main.start()
                #print self.main.error.get()
                """
                if self.main.error.get() == None:
                    pass
                else:
                    print 'Error'
                    #raise IOError,u'remotenicovideodlでエラー'
                """
            except:
                print u"エラーが発生しました.\n1度終了してください."
