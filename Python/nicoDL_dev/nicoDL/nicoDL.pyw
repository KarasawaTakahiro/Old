...
class RemoteNicovideoDL(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            while True:
                #####################
                if self.mycheckdir(): break;
                #####################
                elif os.path.splitdrive(self.savedir)[0].upper() == 'E:': break;
                else:
                    # 空き容量不足でループをリセット
                    print u"空き容量不足:\n ->%s" % self.savedir
                    time.sleep(300)
    ...
    def extention(self):
        import extention.extentions as Extention
        extention = Extention

        for item in dir(extention):
            if item.find('nicoDL_')==0:
                print item
                script="""
...
""" % (item)
                #print script
                modulefile = os.path.join(self.cdir, "extention","nicoDL_module.pyw")
                f = open(modulefile, "w")
                f.write(script)
                f.close()
                os.startfile(modulefile)
                print "Move Movieiles"
                os.startfile("C:\Documents and Settings\TOSHIBA\My Documents\My Documents\Takahiro\software\movemoviefile.pyw")
                ##########
                print 'Extention end'
