...
import nicoLoad_various as Various
...
class CancelableNicovideoDownload():
    def __init__(self, nicoId, nicoPw):
        self._nicoid = nicoId
        self._nicopw = nicoPw
        self._flag_cancel = False  # cancel flag
        self._flag_canceled = False  # if success cancel, it turn True

        self.allSize = 0  # movie size
        self.loadedSize = 0  # has loaded movie size
        self.prevLoadedSize = 0  # prev loaded movie size
        self.buffersize = 1024 * 10  # buffer size

    def cancel(self):
        self._flag_cancel = True
        while not self._flag_canceled:
            time.sleep(1)
        return True

    def download(self, movieid, savedir):
        """
        return 0 successed
        return 1 errror
        return 2 canceled
        """
        various = Various.nicoLoad_Various()
        api = NicovideoAPI(movie_id=movieid)

        title = api.get_movie_title()
        strage = api.get_storageURL(nicovideo_id=self.nicoid, nicovideo_pw=self.nicoid)
        # sign in
        opener = urllib2.build_opener(urllib2.HTTPCookieProcessor(cookielib.CookieJar()))
        req = urllib2.Request("https://secure.nicovideo.jp/secure/login")
        req.add_data(urllib.urlencode({"mail":self.nicoid, "password":self.nicopw}))
        opener.open(req)
        opener.open("http://www.nicovideo.jp/watch/"+movieid)
        res = opener.open(strage)
        moviename = various.filenamecheck(title+'.'+res.info().getsubtype())
        # get movie size
        if strage.find('low') == -1:  # 画質high
            self.allSize = api.get_movie_size_high()
        elif strage.find('low') > -1:  # 画質low
            self.allSize = api.get_movie_size_low()
        if self._flag_cancel:
            self._flag_canceled = True
            return 2
        ff = open(os.path.join(savedir, filename), 'wb')
        try:
            while int(ff.tell()) < self.allSize:
                if self._flag_cancel:
                    self._flag_canceled = True
                    return 2
                ff.write(res.read(self.buffersize))
                self.loadedSize = int(ff.tell())
                print "loaded: %d / %d", (self.loadedSize, self.allSize)
        except:
            return 1
        finally:
            ff.close()
            self.prevLoadedSize = self.loadedSize
            self.allSize = 0
            self.loadedSize = 0

        return 0  # 正常終了

    def getMovieSize(self):
        return self.allSize
    def getLoadedSize(self):
        return self.loadedSize
    def getPrevLoadedSize(self):
        return self.prevLoadedSize
    def setBuffersize(self, kb):
        self.buffersize = kb * 1024
